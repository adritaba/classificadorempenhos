...
stats <- stats %>% drop_na() %>% add_row(id=99,pseed=69619,psample=0.03,wfreq=200,wmin=6,fam="logit")
...
for (row in 1:nrow(stats)) {
  ...
  empenhos2022 <- empenhos2022 %>%
    dplyr::mutate(cod_funcao=replace(cod_funcao, cod_funcao != 12, 99), dsc_funcao=replace(dsc_funcao, dsc_funcao != 'EDUCAÇÃO', 'Z-OUTRA')) %>%
  ...
  # Nuvem de palavras
  wordcloud2(data=frequencia[frequencia$dsc_funcao=="EDUCAÇÃO",][c('word','n')], 
             size=1.0,
             color="grey",
             shape='circle')
  ...
  is_educacao <- empenhos2022_join$dsc_funcao == "EDUCAÇÃO"
  
  model <- cv.glmnet(empenhos2022_sparse, is_educacao, family="binomial", parallel=TRUE, trace.it=1, nfolds = 8, alpha = 1)
  plot(model)
  ...
  coefs %>%
    ...
    labs(
      x = NULL,
      title = "Coeficientes que aumentam/diminuem a probabilidade de ser da classe Educação"
    ) +
    ylab("estimativa")
  ...
  curva_roc <- classif_real %>%
    ...
    labs(
      title = "Curva ROC",
      subtitle = "Predição de classificação de empenhos em Educação",
    ) +
...
#write.csv2(x=hist,file="C:\\Users\\ataba\\MBA\\ClassificadorEmpenhos\\plots\\hist_edu_otimo.csv")
#write.csv2(x=hist_saude,file="C:\\Users\\ataba\\MBA\\ClassificadorEmpenhos\\plots\\hist_saude.csv")
